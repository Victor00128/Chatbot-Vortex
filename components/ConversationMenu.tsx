...
const ConversationMenu: React.FC<ConversationMenuProps> = ({ onClose, onRename, onDownload, onDelete }) => {
  ...
  return (
    <div
      ref={menuRef}
      className="absolute top-8 right-2 w-48 bg-zinc-800 border border-zinc-700 rounded-lg shadow-2xl z-40 animate-message-in"
      style={{ animationDuration: '0.15s' }}
    >
      <ul className="text-sm text-gray-200 p-1">
        <li onClick={() => handleAction(onRename)} className="flex items-center gap-3 px-3 py-2 hover:bg-zinc-700 rounded-md cursor-pointer">
          <RenameIcon /> Renombrar
        </li>
        <li onClick={() => handleAction(onDownload)} className="flex items-center gap-3 px-3 py-2 hover:bg-zinc-700 rounded-md cursor-pointer">
          <DownloadIcon /> Descargar Chat
        </li>
        <li onClick={() => handleAction(onDelete)} className="flex items-center gap-3 px-3 py-2 hover:bg-zinc-700 text-red-400 rounded-md cursor-pointer">
